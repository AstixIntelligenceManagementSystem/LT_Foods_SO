...
public class DBAdapterLtFoods {
   ...
   private static final String DATABASE_CREATE_TABLE_1User = "create table tblUserAuthenticationMstr (flgUserAuthenticated text null,flgAppStatus text null,DisplayMessage text null,flgValidApplication text null,MessageForInvalid text null);";
    ...
    private static final String DATABASE_CREATE_TABLE_tblDistribtorMstr = "create table tblDistribtorMstr " +
            "(DBRNodeID int null,DistributorNodeType int null,Distributor text null, flgRemap int null," +
            "ContactNumber text null,EmailID text null);";
    ...
    private static final String DATABASE_CREATE_TABLE_tblDistributorMapping="create table tblDistributorMapping(" +
            ...
            "flgLocationServicesOnOff int null,flgGPSOnOff int null,flgNetworkOnOff int null,flgFusedOnOff int null," +
            "flgInternetOnOffWhileLocationTracking int null,flgRestart int null,MapAddress text null,MapCity text null," +
            "MapPinCode text null,MapState text null,CityId text null,StateId text null," +
            "PhoneNo text null,EmailID text null);";
  ...
  /*  public long savetblUserAuthenticationMstr(String flgUserAuthenticated)
    ...
    public long savetblUserAuthenticationMstr(String flgUserAuthenticated,String flgAppStatus,String DisplayMessage,
...
// StoreSectionCount,int flgApproveOrRejectOrNoActionOrReVisit,String SOLatCode,String SOLongCode,int flgStoreVisitMode,String VisitStartTS,String VisitEndTS,String LocProvider,String Accuracy,String BateryLeftStatus,int IsStoreDataCompleteSaved,String PaymentStage,int flgLocationTrackEnabled)
    ...
    {
        String StoreCategoryType=getChannelGroupIdOptIdForAddingStore();
        open();
                    ...
                    for (int i = 0; i <= (cursor.getCount() - 1); i++)
                    {
                        if(i==0)
                        {
                            if(flgForReportOrStoreListActivity==0) {
                                ...
                                {
                                    hmapCoverageArea.put("All Merchandiser/Coverage Area", "0");
                                }
                                else {
   ...
   /* public LinkedHashMap<String, String> fnGeStoreListAllForSO(int CoverageAreadID,int RouteID)
    ...
    public LinkedHashMap<String, String> fnGeStoreListAllForSO(int CoverageAreadID,int RouteID)
    {
        ...
        try {
            ...
            if(CoverageAreadID==0 && RouteID==0)
            {
                cursor = db.rawQuery("SELECT StoreID,StoreName,DateAdded,CoverageAreaID,RouteNodeID,StoreCategoryType,StoreSectionCount,flgApproveOrRejectOrNoActionOrReVisit,Sstat,flgOldNewStore,flgRemap from tblPreAddedStores Where  (Sstat<>4 AND  flgApproveOrRejectOrNoActionOrReVisit<>1 AND flgApproveOrRejectOrNoActionOrReVisit<>2 AND flgApproveOrRejectOrNoActionOrReVisit<>3) OR (Sstat=4 AND flgOldNewStore=1) ORDER BY DistanceNear", null);
            }
            else if(CoverageAreadID!=0 && RouteID!=0)
            {
                cursor = db.rawQuery("SELECT StoreID,StoreName,DateAdded,CoverageAreaID,RouteNodeID,StoreCategoryType,StoreSectionCount,flgApproveOrRejectOrNoActionOrReVisit,Sstat,flgOldNewStore,flgRemap from tblPreAddedStores WHERE (CoverageAreadID="+CoverageAreadID+" AND RouteID="+RouteID+" and  Sstat<>4 AND  flgApproveOrRejectOrNoActionOrReVisit<>1 AND flgApproveOrRejectOrNoActionOrReVisit<>2 AND flgApproveOrRejectOrNoActionOrReVisit<>3) OR (Sstat=4 AND flgOldNewStore=1)  ORDER BY DistanceNear", null);//
            }
            else if(CoverageAreadID!=0 && RouteID==0)
            {
                cursor = db.rawQuery("SELECT StoreID,StoreName,DateAdded,CoverageAreaID,RouteNodeID,StoreCategoryType,StoreSectionCount,flgApproveOrRejectOrNoActionOrReVisit,Sstat,flgOldNewStore,flgRemap from tblPreAddedStores WHERE (CoverageAreadID="+CoverageAreadID+" AND  Sstat<>4 AND  flgApproveOrRejectOrNoActionOrReVisit<>1 AND flgApproveOrRejectOrNoActionOrReVisit<>2 AND flgApproveOrRejectOrNoActionOrReVisit<>3) OR (Sstat=4 AND flgOldNewStore=1)  ORDER BY DistanceNear", null);//
            }
            else if(CoverageAreadID==0 && RouteID!=0)
            {
                cursor = db.rawQuery("SELECT StoreID,StoreName,DateAdded,CoverageAreaID,RouteNodeID,StoreCategoryType,StoreSectionCount,flgApproveOrRejectOrNoActionOrReVisit,Sstat,flgOldNewStore,flgRemap from tblPreAddedStores WHERE (RouteID="+RouteID+" AND  Sstat<>4 AND  flgApproveOrRejectOrNoActionOrReVisit<>1 AND flgApproveOrRejectOrNoActionOrReVisit<>2 AND flgApproveOrRejectOrNoActionOrReVisit<>3) OR (Sstat=4 AND flgOldNewStore=1)  ORDER BY DistanceNear", null);//
            }
    ...
    //tblPDAQuestOptionValuesDependentMstr
...
//tblQuestionDependentMstr(QuestionID int null,OptionID int null,DependentQuestionID int null,GrpID int null,GrpDepQuestID int null);";
    ...
    public void fndeleteSbumittedStoreList(int Sstat)
    {
      ...
      /*  open();
        ...
        db.execSQL("DELETE FROM tblDistributorMapping WHERE Sstat="+ Sstat);
...
//tblQuestIDForOutChannel(GrpQstId int null,QuestID int null,OptID text null,SectionCount int null);";
    ...
    public int CheckTotalStoreCount() throws IOException
    {
        ...
        try
        {
            cursorE2 = db.rawQuery("SELECT flgStoreOrder from tblPreAddedStores order by flgStoreOrder descr limit 1", null);
    ...
    public LinkedHashMap<String,String> getDistributorContactInfo()
    {
        LinkedHashMap<String,String> strStoreTypeNamesDetais =new LinkedHashMap<>();
        try {
            Cursor cursor2 = db.rawQuery("SELECT DBRNodeID,DistributorNodeType,ContactNumber,EmailID FROM tblDistribtorMstr", null);
            ...
            if(cursor2.getCount()>0)
            {
                if (cursor2.moveToFirst())
                {
                    for (int i = 0; i < cursor2.getCount(); i++)
                    {
                        strStoreTypeNamesDetais.put(cursor2.getString(0)+"^"+cursor2.getString(1),cursor2.getString(2)+"^"+cursor2.getString(3));
                        cursor2.moveToNext();
                    }
                }
            }
    ...
    public long savetblDistributorMappingData(String DistribtrUniqueId,String  DistribtrId ,String DistributorNodeType ,
                                              ...
                                              int Sstat,int flgLocationServicesOnOff,int flgGPSOnOff,
                                              int flgNetworkOnOff,int flgFusedOnOff,
                                              int flgInternetOnOffWhileLocationTracking,int flgRestart
    ...
    public void deleteDistributorMap()
    {
        open();
